...
namespace ZenTimings.Helpers
{
    internal class DriverCleaner
    {
        ...
        internal static void Cleanup(NotificationLevel notificationLevel)
        {
            ...
            try
            {
                ...
                if (scm == IntPtr.Zero)
                {
                    ...
                    if (notificationLevel > NotificationLevel.None)
                        DriverCleanerNotification.Show("ZenTimings", "Could not open the Service Control Manager. Error: " + error, true);
                ...
                if (service == IntPtr.Zero)
                {
                    ...
                    if (error == ERROR_SERVICE_DOES_NOT_EXIST)
                    {
                        if (TryDeleteDriverFile())
                        {
                            if (notificationLevel > NotificationLevel.Error)
                                DriverCleanerNotification.Show("ZenTimings", "The inpoutx64 driver was successfully removed.");
                        }
                        else
                        {
                            if (notificationLevel > NotificationLevel.None)
                                DriverCleanerNotification.Show("ZenTimings", "The service was removed, but the driver file could not be deleted.", true);
                        }
                    ...
                    if (notificationLevel > NotificationLevel.None)
                        DriverCleanerNotification.Show("ZenTimings", "Could not open the inpoutx64 service. Error: " + error, true);
                ...
                if (!QueryStatus(service, out status))
                {
                    int error = Marshal.GetLastWin32Error();
                    if (notificationLevel > NotificationLevel.None)
                        DriverCleanerNotification.Show("ZenTimings", "Could not query the inpoutx64 service. Error: " + error, true);
                ...
                {
                    if (notificationLevel > NotificationLevel.None)
                        DriverCleanerNotification.Show("ZenTimings", "The inpoutx64 driver is already being stopped or is currently in use. Cleanup was aborted.", true);
                ...
                if (status.dwCurrentState == SERVICE_STOPPED)
                {
                    ...
                    {
                        if (notificationLevel > NotificationLevel.None)
                            DriverCleanerNotification.Show("ZenTimings", "The inpoutx64 service was removed, but the driver file could not be deleted.", true);
                ...
                if (status.dwCurrentState == SERVICE_RUNNING)
                {
                    ...
                    if (!ControlService(service, SERVICE_CONTROL_STOP, out controlStatus))
                    {
                        ...
                        {
                            if (notificationLevel > NotificationLevel.None)
                                DriverCleanerNotification.Show("ZenTimings", "Could not stop the inpoutx64 driver. Error: " + error, true);
                ...
                {
                    if (notificationLevel > NotificationLevel.None)
                        DriverCleanerNotification.Show("ZenTimings", "The inpoutx64 driver could not be stopped. It may be in use by another application. Cleanup was aborted.", true);
                ...
                {
                    if (notificationLevel > NotificationLevel.None)
                        DriverCleanerNotification.Show("ZenTimings", "The inpoutx64 driver stopped, but the service could not be deleted.", true);
